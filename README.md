...
|VIEW | - **form.jsp** : 메인 페이지,<br> - **AllView.jsp** : DB로부터 입퇴실 기록 받아와서 form.jsp에 뿌리는 JSP,<br> - **register.jsp** : 입실 페이지(입실 등록),<br>- **out.jsp** : 퇴실 확인 페이지,<br> - **login.jsp** : 관리자 로그인하는 페이지,<br> - **msgView.jsp** : 서버에서 전달하는 메시지를 받아서 client에게 보여주는 페이지 ,<br> - **errorView.jsp** : 서버에서 받은 error메시지를 client에 보여주는 페이지, - **singup.jsp** : 관리자 회원가입 페이지<br> - **footer.jsp** : 하단 정보표시란->include로 모듈화 구현 중.....<br> - **boardmain.jsp** : bootstap을 사용해 게시판 구현 중...<br> - **IdCheckForm.jsp** : 회원가입 시 아이디 중복 확인 페이지 (bootstap) <br> |
...
| 파일 종류 | 파일 설명  |
|--|--|
| JavaScript| - **form.js** : ```button```에 ```hover```기능을 부여한다. 윈도우 창 크기에 따른 상단 navbar크기를 조정한다. 그리고 입실과 퇴실 정보를 ```ajax```기능으로 form.jsp에 비동기로 전달한다.<br> - **time.js** : 화면에서 년, 월, 일, 시간, 표시 함수를 가지는 javaScript <br> - **signup.js** :  회원가입 시 규정사항 기능 JavaScript <br> - **majorgrahp.js** : (구현 중)데이터베이스에서 가장 이용 횟수가 많은 top7 학과를 나타내는 Google chart <br> |
| CSS | - **login.css** <br> - **form.css**<br> - **register.css**<br> - **menu.css**<br> - **common.css**<br> - **footer.css**<br> - **navbar.css** :<br> - **signup.css** : 회원가입 페이지 CSS <br> - **top.css** : 구현 중...<br> |
