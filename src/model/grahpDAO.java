...
public class grahpDAO {
	...
	public static String counting(String no) throws Exception{
		...
		try {
			...
			case "4" : 
				rs = stmt.executeQuery("SELECT COUNT(*) AS count FROM register WHERE inout = '입실' AND today = '"+ day +"'");
				while(rs.next()) {
					System.out.println("데이터베이스에서 반환되는 값 : " + rs.getString("count"));
					usedcount = Integer.parseInt(rs.getString("count"));
				}
				break;
			...
			if(no=="4") {
				usedcount = 9 - usedcount; 
				System.out.println("빈 자리 개수는 " + usedcount);
			}else{
				usedcount = rs.getRow();
			}			
		}finally {
		...
		return Integer.toString(usedcount);
	}
